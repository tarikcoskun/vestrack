...
function TabsProvider({ value, setValue, onValueChange, children }: React.PropsWithChildren<TabsValue>) {
...
const TabsRoot = forwardRef<HTMLDivElement, TabsProps>((props, forwardedRef) => {
  const { defaultValue, orientation = "horizontal", className, children, ...tabsProps } = props;
...
const TabsList = forwardRef<HTMLDivElement, TabsListProps>((props, forwardedRef) => {
  ...
  useEffect(() => {
    const listener = () => {
      ...
      if (parentRect.height - rect.top <= 0) setSticked(true);
